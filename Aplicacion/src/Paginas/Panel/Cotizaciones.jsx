...
export function Cotizaciones() {
  ...
  const generarFactura=async cotizacion=>{
    try {
      setError('');setCorrecto('')
      const respuesta=await Solicitar(`/cotizaciones/${cotizacion.id}/factura`,{method:'POST',body:'{}'})
      setCorrecto(`Factura ${respuesta.numero} generada`);await cargar()
  ...
  return <div className="pagina">
    ...
    <div className="tabla"><table>
      ...
      <tbody>{cotizaciones.map(c=><tr key={c.id}>
        ...
        <td><div className="accionesfila vertical"><button className="secundario" onClick={()=>{setSeleccion(c);setAccion('detalle')}}>Ver detalle</button>{c.comprobanteen&&['Administrador','AsesorVentas'].includes(usuario.rol)&&<button className="secundario" onClick={()=>Descargar(`/cotizaciones/${c.id}/comprobante`,c.comprobantenombre||'comprobante')}>Comprobante</button>}{c.estado==='Entregada'&&!c.facturacodigo&&<button className="secundario" onClick={()=>generarFactura(c)}>Generar factura</button>}{c.facturacodigo&&<><button className="secundario" onClick={()=>Descargar(`/cotizaciones/${c.id}/factura/pdf`,`${c.facturanumero}.pdf`)}>Descargar factura</button><a className="secundario botonverificacion" href={`/?verificarfactura=${c.facturacodigo}`} target="_blank" rel="noreferrer">Verificar factura</a></>}{acciones(c).map(([id,nombre])=><button key={id} className={id.includes('rechazar')?'secundario peligrotexto':'secundario'} onClick={()=>abrir(c,id)}>{nombre}</button>)}<button className="secundario" onClick={()=>verHistorial(c)}>Historial</button></div></td>
      </tr>)}</tbody>
    </table></div>
