...
export function Usuarios() {
  ...
  const guardar=async evento=>{
    ...
    try {
      ...
      setModal(false);setFormulario(Vacio);setCorrecto(`Usuario creado. Correo: ${respuesta.correoestado}`);setCredenciales(respuesta.demostracion||null);await cargar()
  ...
  const reenviar=async usuario=>{
    try {
      const respuesta=await Solicitar(`/usuarios/${usuario.id}/reenviar`,{method:'POST'})
      setCorrecto(`Código renovado. Correo: ${respuesta.correoestado}`);setCredenciales(respuesta.demostracion||null)
    } catch (fallo) { setError(fallo.message) }
  }

  return <div className="pagina">
    <Titulo titulo="Trabajadores" descripcion="Creación, verificación por correo, roles y bloqueo inmediato" accion={<button className="principal" onClick={()=>setModal(true)}>Nuevo trabajador</button>}/>
    <Mensaje error={error} correcto={correcto}/>
    {credenciales&&<div className="panel credenciales"><h3>Credenciales de demostración</h3><p>Solo aparecen en modo desarrollo. En producción se envían por correo.</p><code>Código: {credenciales.codigo}</code><code>Clave temporal: {credenciales.clavetemporal||credenciales.temporal}</code></div>}
    ...
    {modal&&<Modal titulo="Nuevo trabajador" cerrar={()=>setModal(false)}><form onSubmit={guardar}>
      ...
      <button className="principal">Crear y enviar código</button>
    </form></Modal>}
  </div>
}